...
public class EmbeddedNeo4jDatabase {
    ...
    /**
     * Maps protein list onto database and prints out the mapped nodes
     * @param pathtoProtiens
     * @return
     * @throws IOException
     */
    public HashMap<Long, Integer> mapProteins(File pathtoProtiens) throws IOException {

        GraphDatabaseService graphDb = new GraphDatabaseFactory().newEmbeddedDatabase(databaseDir);

        try (Transaction tx = graphDb.beginTx()) {

            // get the first UID from each line in File
            BufferedReader BR = new BufferedReader(new FileReader(pathtoProtiens));
            String line = "";
            ...
            for (String uid : UIDList) {
                ...
                if (node == null) {
                    ...
                    for (Relationship relationship : relationships) {
                        Long end = relationship.getEndNode().getId();
                        if (Match.containsKey(end)) {
                            Integer count = Match.get(end);
                            count++;
                            Match.put(end, count);
                        } else {
                            Integer count = 1;
            ...
            //Create File to write Matches to
            FileWriter fstream = new FileWriter(outputFile + "/matched.tsv");
            BufferedWriter out = new BufferedWriter(fstream);

            Iterator<Map.Entry<Long, Integer>> it = Match.entrySet().iterator();
            out.write("ID\tvalue\n");
            while (it.hasNext()) {
            ...
            tx.success();
            graphDb.shutdown();
            System.out.println("Matched Unique UID count: " + counter);
            System.out.println("Count of UniProt ID's that did not match: " + uidsNotInDb);
            return Match;
        }
